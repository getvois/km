...
/**
 * Repository class for the PlacePage
 */
class PlacePageRepository extends AbstractArticlePageRepository
{

    /**
     * Returns an array of all PlacePages
     *
     * @param string $lang
     * @param int    $offset
     * @param int    $limit
    ...
    public function getArticlesQuery($lang = null, $offset, $limit)
    {
        ...
        $rsm->addRootEntityFromClassMetadata('Sandbox\WebsiteBundle\Entity\Place\PlacePage', 'qp');

        $query = "SELECT";
        $query .= " article.*";
        $query .= " FROM";
        $query .= " sb_place_pages as article";
        $query .= " INNER JOIN";
        ...
        $query .= " n.deleted = 0";
        $query .= " AND";
        $query .= " n.ref_entity_name = 'Sandbox\\\\WebsiteBundle\\\\Entity\\\\Place\\\\PlacePage'";
        $query .= " AND";
        $query .= " nt.online = 1 ";
        if ($lang) {
            $query .= " AND";
